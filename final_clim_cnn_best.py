...
class CNN(torch.nn.Module):
	def __init__(self):
		...
		self.fc1 = nn.Linear(576, 148)
		self.fc2 = nn.Linear(148, 18)
...
for epoch in range(n_epochs):
	...
	loss = loss_fn(targets_array, prd_array)
	print("Epoch %d: model loss %.2f" % (epoch, loss))
	if average_train_loss < best_loss:
		best_loss = average_train_loss
		best_epoch = epoch
		checkpoint(model, "C:\\Users\\willi\\OneDrive\\Documents\\Climate Research\\Data Estimation\\Best Clim\\clim_cnn_best_model_n.pth")
	elif epoch - best_epoch > early_stop_thresh:
		print("Early stopped training at epoch %d" % epoch)
		break  # terminate the training loop

resume(model, "C:\\Users\\willi\\OneDrive\\Documents\\Climate Research\\Data Estimation\\Best Clim\\clim_cnn_latest.pth")
print(model)
...
prd_df.to_csv('Future Climate CNN_N.csv', index=False)