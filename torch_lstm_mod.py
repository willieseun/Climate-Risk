...
X_train = np.reshape(X_train, (X_train.shape[0], 8, 19))
X_test = np.reshape(X_test, (X_test.shape[0], 8, 19))
...
class LSTMModel(nn.Module):
	def __init__(self):
		...
		self.lstm1 = nn.LSTM(19, 190)
		self.lstm2 = nn.LSTM(190, 76)
...
X_rs = np.reshape(X_rs, (X_rs.shape[0], 8, 19))
...
prd_df[lst] = Y_rs
prd_df.to_csv('Future Maize Production 97.csv', index=False)